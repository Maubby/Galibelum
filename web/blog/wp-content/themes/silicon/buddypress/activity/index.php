...
<div id="buddypress">
	...
	<div class="item-list-tabs activity-type-tabs si-bp-sub-nav" aria-label="<?php esc_attr_e( 'Sitewide activities navigation', 'silicon' ); ?>" role="navigation">
		<ul>
			<?php
			...
			<li class="selected" id="activity-all">
                ...
                ><?php printf( esc_html__( 'All Members %s', 'silicon' ), '<span>' . bp_get_total_member_count() . '</span>' ); ?></a>
            </li>
					...
					<?php if ( bp_get_total_friend_count( bp_loggedin_user_id() ) ) : ?>
						...
						<li id="activity-friends">
                            <a href="<?php echo bp_loggedin_user_domain() . bp_get_activity_slug() . '/' . bp_get_friends_slug() . '/'; ?>"
                               class="btn btn-solid btn-pill btn-primary"
                            ><?php printf( esc_html__( 'My Friends %s', 'silicon' ), '<span>' . bp_get_total_friend_count( bp_loggedin_user_id() ) . '</span>' ); ?></a>
                        </li>
				...
				<?php if ( bp_is_active( 'groups' ) ) : ?>
					...
					<?php if ( bp_get_total_group_count_for_user( bp_loggedin_user_id() ) ) : ?>
						...
						<li id="activity-groups">
                            <a href="<?php echo bp_loggedin_user_domain() . bp_get_activity_slug() . '/' . bp_get_groups_slug() . '/'; ?>"
                               class="btn btn-solid btn-pill btn-primary"
				...
				<?php if ( bp_get_total_favorite_count_for_user( bp_loggedin_user_id() ) ) : ?>
					...
					<li id="activity-favorites">
                        <a href="<?php echo bp_loggedin_user_domain() . bp_get_activity_slug() . '/favorites/'; ?>"
                           class="btn btn-solid btn-pill btn-primary"
                        ><?php printf( esc_html__( 'My Favorites %s', 'silicon' ), '<span>' . bp_get_total_favorite_count_for_user( bp_loggedin_user_id() ) . '</span>' ); ?></a>
                    </li>
				...
				<?php if ( bp_activity_do_mentions() ) : ?>
					...
					<li id="activity-mentions">
                        <a href="<?php echo bp_loggedin_user_domain() . bp_get_activity_slug() . '/mentions/'; ?>"
                           class="btn btn-solid btn-pill btn-primary"
                        ><?php esc_html_e( 'Mentions', 'silicon' ); if ( bp_get_total_mention_count_for_user( bp_loggedin_user_id() ) ) : ?> <strong><span><?php printf( _nx( '%s new', '%s new', bp_get_total_mention_count_for_user( bp_loggedin_user_id() ), 'Number of new activity mentions', 'silicon' ), bp_get_total_mention_count_for_user( bp_loggedin_user_id() ) ); ?></span></strong><?php endif; ?></a>
                    </li>
	...
	<div class="item-list-tabs no-ajax si-bp-sub-nav" id="subnav" aria-label="<?php esc_attr_e( 'Activity secondary navigation', 'silicon' ); ?>" role="navigation">
		<ul>
			<li class="feed">
                <a href="<?php bp_sitewide_activity_feed_link(); ?>"
                   class="btn btn-solid btn-pill btn-primary"
                   title="<?php esc_attr_e( 'RSS Feed', 'silicon' ); ?>"
                ><?php esc_html_e( 'RSS', 'silicon' ); ?></a>
            </li>
