	...
	<ul>
		...
		<?php if ( !bp_is_current_action( 'requests' ) ) : ?>
			...
			<li id="members-order-select" class="last filter">
				...
				<label for="members-friends"><?php esc_html_e( 'Order By:', 'silicon' ); ?></label>
				<select id="members-friends">
					<option value="active"><?php esc_html_e( 'Last Active', 'silicon' ); ?></option>
					<option value="newest"><?php esc_html_e( 'Newest Registered', 'silicon' ); ?></option>
	...
	case 'my-friends' :
		...
		<?php if (is_user_logged_in() ) : ?>
			<h2 class="bp-screen-reader-text"><?php
				...
				esc_html_e( 'My friends', 'silicon' );
			?></h2>
		<?php else : ?>
			<h2 class="bp-screen-reader-text"><?php
				/* translators: accessibility text */
				esc_html_e( 'Friends', 'silicon' );
			?></h2>
		<?php endif ?>
