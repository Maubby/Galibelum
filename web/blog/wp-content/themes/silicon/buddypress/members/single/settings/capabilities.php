...
<form action="<?php echo bp_displayed_user_domain() . bp_get_settings_slug() . '/capabilities/'; ?>" name="account-capabilities-form"
      id="account-capabilities-form" class="standard-form" method="post">
	...
	<label for="user-spammer">
		<input type="checkbox" name="user-spammer" id="user-spammer" value="1" <?php checked( bp_is_user_spammer( bp_displayed_user_id() ) ); ?> />
		 <?php esc_html_e( 'This user is a spammer.', 'silicon' ); ?>
	</label>

	<div class="submit">
        <button type="submit" id="capabilities-submit"
                name="capabilities-submit" class="btn btn-solid btn-pill btn-default"
        ><?php esc_html_e( 'Save', 'silicon' ); ?></button>
	</div>
