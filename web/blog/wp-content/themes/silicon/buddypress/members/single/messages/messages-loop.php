...
<?php if ( bp_has_message_threads( bp_ajax_querystring( 'messages' ) ) ) : ?>
	...
	<h2 class="bp-screen-reader-text"><?php
		...
		esc_html_e( 'Starred messages', 'silicon' );
	?></h2>
		...
		<table id="message-threads" class="messages-notices">
			...
			<thead>
				<tr>
					...
					?></label></th>
					<th scope="col" class="thread-from"><?php esc_html_e( 'From', 'silicon' ); ?></th>
					<th scope="col" class="thread-info"><?php esc_html_e( 'Subject', 'silicon' ); ?></th>
					...
					<?php if ( bp_is_active( 'messages', 'star' ) ) : ?>
						<th scope="col" class="thread-star"><span class="message-action-star"><span class="icon"></span> <span class="screen-reader-text"><?php
							/* translators: accessibility text */
							esc_html_e( 'Star', 'silicon' );
						?></span></span></th>
					<?php endif; ?>

					<th scope="col" class="thread-options"><?php esc_html_e( 'Actions', 'silicon' ); ?></th>
				</tr>
			</thead>
					...
					<tr id="m-<?php bp_message_thread_id(); ?>" class="<?php bp_message_css_class(); ?><?php if ( bp_message_thread_has_unread() ) : ?> unread<?php else: ?> read<?php endif; ?>">
						<td class="bulk-select-check">
							<label for="bp-message-thread-<?php bp_message_thread_id(); ?>"><input type="checkbox" name="message_ids[]" id="bp-message-thread-<?php bp_message_thread_id(); ?>" class="message-check" value="<?php bp_message_thread_id(); ?>" /><span class="bp-screen-reader-text"><?php
								/* translators: accessibility text */
								esc_html_e( 'Select this message', 'silicon' );
							?></span></label>
						</td>
						...
						<?php if ( 'sentbox' != bp_current_action() ) : ?>
							<td class="thread-from">
								...
								<span class="from"><?php esc_html_e( 'From:', 'silicon' ); ?></span> <?php bp_message_thread_from(); ?>
						...
						<?php else: ?>
							<td class="thread-from">
								<?php bp_message_thread_avatar( array( 'width' => 25, 'height' => 25 ) ); ?>
								<span class="to"><?php esc_html_e( 'To:', 'silicon' ); ?></span> <?php bp_message_thread_to(); ?>
								<?php bp_message_thread_total_and_unread_count(); ?>
								<span class="activity"><?php bp_message_thread_last_post_date(); ?></span>
							</td>
						<?php endif; ?>
						...
						<td class="thread-info">
							<p><a href="<?php bp_message_thread_view_link(); ?>" class="navi-link-color navi-link-hover-color" title="<?php esc_attr_e( "View Message", 'silicon' ); ?>"><?php bp_message_thread_subject(); ?></a></p>
						...
						<td class="thread-options">
							<?php if ( bp_message_thread_has_unread() ) : ?>
								<a class="btn btn-solid btn-pill btn-default read" href="<?php bp_the_message_thread_mark_read_url();?>"><?php esc_html_e( 'Read', 'silicon' ); ?></a>
							<?php else : ?>
								<a class="btn btn-link btn-pill btn-primary unread" href="<?php bp_the_message_thread_mark_unread_url();?>"><?php esc_html_e( 'Unread', 'silicon' ); ?></a>
							<?php endif; ?>

							<a class="btn btn-link btn-pill btn-warning delete" href="<?php bp_message_thread_delete_link(); ?>"><?php esc_html_e( 'Delete', 'silicon' ); ?></a>
...
<?php else: ?>
	...
	<div id="message" class="info">
		<p><?php esc_html_e( 'Sorry, no messages were found.', 'silicon' ); ?></p>
	</div>
